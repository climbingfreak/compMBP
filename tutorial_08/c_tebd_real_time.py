    ...
    for n in range(N_steps):
        for k in [0, 1]:  # even, odd
            for i_bond in range(k, Nbonds, 2):
                ...
                magnetization = np.sum(psi.site_expectation_value(sz))
                magnetizations.append(magnetization)
                entropy = psi.entanglement_entropy()
                entropy = sum(entropy[:len(entropy)//2])
                entropies.append(entropy)
    # done
    return magnetizations, entropies
...
def example_TEBD_gs_finite(L, J, g, t):
    ...
    for dt in [0.1, 0.01, 0.001, 1.e-4, 1.e-5][:1]:
        U_bonds = calc_U_bonds(model, dt)
        magnetizations, entropies = run_TEBD(psi, U_bonds, N_steps=int(t/dt), chi_max=30, eps=1.e-10)
        E = model.energy(psi)
    ...
    return E, psi, model, magnetizations, entropies
